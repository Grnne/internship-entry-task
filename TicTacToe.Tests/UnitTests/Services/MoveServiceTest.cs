...
public class MoveServiceTests : IDisposable
{
            ...
            new object[]
            {
                ...
                new Action<TicTacToeDbContext>(ctx =>
                {
                    var game = new Game
                    {
                        ...
                        BoardWidth = 3,
                        WinLength = 3,
                        Cells = [new Cell { X = 0, Y = 0, CellState = CellState.Empty }]
                    };
                    ctx.Games.Add(game);
                    ctx.SaveChanges();
                }),
            ...
            new object[]
            {
                ...
                new Action<TicTacToeDbContext>(ctx =>
                {
                    var game = new Game
                    {
                        ...
                        BoardWidth = 3,
                        WinLength = 3,
                        Cells = [new Cell { X = 0, Y = 0, CellState = CellState.X }]
                    };
                    ctx.Games.Add(game);
                    ctx.SaveChanges();
                }),
    ...
    [Fact]
    public async Task CreateAsync_ValidMove_ProcessesMoveSuccessfully()
    {
        // Arrange
        var game = new Game
        {
        ...
        // Extra: check cell and move records
    ...
    [Fact]
    public async Task CreateAsync_ConcurrentMovesOnSameCell_OnlyOneSucceeds()
    {
        // Arrange
        var game = new Game
        {
    ...
    public async Task CreateAsync_InvalidMove_ReturnsExpectedError(
        CreateMoveDto? dto,
